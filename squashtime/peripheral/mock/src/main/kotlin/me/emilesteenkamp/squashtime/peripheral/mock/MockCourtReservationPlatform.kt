...
class MockCourtReservationPlatform
    ...
    constructor(
        ...
        override suspend fun authenticate(
            ...
            val actualPassword =
                dataSource.playerUserNameToPasswordMap[player.userName]
        ...
        class Session(
            ...
            override suspend fun fetchSchedule(date: LocalDate): CourtReservationPlatform.Session.FetchScheduleResult {
                val mutableSchedule =
                    dataSource.dateToMutableScheduleMap[date] ?: return CourtReservationPlatform.Session.FetchScheduleResult.Failed
            ...
            override suspend fun reserveCourt(
                ...
                val schedule =
                    dataSource.dateToMutableScheduleMap[date] ?: return CourtReservationPlatform.Session.ReserveCourtResult.Failed
                val timeslotList = schedule[courtIdentifier] ?: return CourtReservationPlatform.Session.ReserveCourtResult.Failed
                val timeslot =
                    timeslotList.firstOrNull { it.identifier == timeslot.identifier }
        ...
        class DataSource
            ...
            constructor() {
                ...
                private fun defaultMutableSchedulesForNextSevenDays() =
                    buildMap<LocalDate, MutableSchedule> {
                        dateList.forEach { put(it, defaultMutableSchedule()) }
                    }

                private fun defaultMutableSchedule(): MutableSchedule =
                    buildMap {
                        courtIdentifierSet.forEach { put(it, defaultMutableTimeslotList()) }
                    }

                private fun defaultMutableTimeslotList(): List<MutableTimeslot> =
                    buildList {
                ...
                companion object {
                    private val courtIdentifierSet =
                        setOf(
                            CourtIdentifier("1"),
                            CourtIdentifier("2"),
                            CourtIdentifier("3"),
        ...
        companion object {
            fun MutableSchedule.toSchedule(): Schedule =
                this.mapValues { (_, mutableTimeslotList) ->
                    mutableTimeslotList.map { mutableTimeslot ->
                        mutableTimeslot.toTimeslot()
                    }
                }

            fun DataSource.MutableTimeslot.toTimeslot(): Timeslot =
                Timeslot(
