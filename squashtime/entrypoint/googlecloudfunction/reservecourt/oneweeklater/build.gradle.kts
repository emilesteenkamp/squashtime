...
tasks.register<JavaExec>("runFunction") {
    ...
    args(
        "--target", "me.emilesteenkamp.squashtime.entrypoint.googlecloudfunction.reservecourt.oneweeklater.ReserveCourtOneWeekLaterFunctionEntryPoint",
        "--port", "8080"
    )
    doFirst {
        args(
...
jib {
    ...
    to {
        image = "europe-west4-docker.pkg.dev/squash-time-471311/squash-time/reserve-court-one-week-later:latest"
    }
    container {
        mainClass = "com.google.cloud.functions.invoker.runner.Invoker"
        ports = listOf("8080")
        environment = mapOf(
            "FUNCTION_TARGET" to "me.emilesteenkamp.squashtime.entrypoint.googlecloudfunction.reservecourt.oneweeklater.ReserveCourtOneWeekLaterFunctionEntryPoint"
        )
    }
}